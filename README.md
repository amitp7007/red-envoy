...
```sh
make docker-build docker-push IMG=<some-registry>/red-envoy:tag
```
...
```sh
make deploy IMG=<some-registry>/red-envoy:tag
```
...
```sh
make build-installer IMG=<some-registry>/red-envoy:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/red-envoy/<tag or branch>/dist/install.yaml
```
